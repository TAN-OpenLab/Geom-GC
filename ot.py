...
class ObliviousTransfer:
    ...
    def get_result(self, a_inputs, b_garbled_values):
        """Send Alice's inputs and retrieve Bob's result of evaluation.

        Args:
            a_inputs: A dict mapping Alice's wires to garbled inputs.
            b_garbled_values: A dict mapping each Bob's wire to a pair garbled values.

        Returns:
            The result of the evaluation.
        """
        logging.debug("Sending inputs to Bob")
        self.socket.send(a_inputs)   #send a-input

        for _ in range(len(b_garbled_values)):
            ...
            if self.enabled:  # perform oblivious transfer
                pair = (pickle.dumps(b_garbled_values[w][0]), pickle.dumps(b_garbled_values[w][1]))
                self.ot_generator(pair)
            else:
                to_send = (b_garbled_values[w][0], b_garbled_values[w][1])
                self.socket.send(to_send)

        return self.socket.receive()

    def send_result(self, circuit, g_tables, b_inputs):
        """Evaluate circuit and send the result to Alice.

        Args:
            circuit: A dict containing circuit spec.
            g_tables: Garbled tables.
            pbits_out: p-bits of outputs.
            b_inputs: A dict mapping Bob's wires to (clear) input bits.
        """
        # map from Alice's wires to garbled inputs
        a_inputs = self.socket.receive()   ##receive a-input
        # map from Bob's wires to garbled inputs
        b_inputs_encr = {}
        ...
        for w, b_input in b_inputs.items():
            ...
            self.socket.send(w)   #send w

            if self.enabled:
                b_inputs_encr[w] = pickle.loads(self.ot_evaluator(b_input))
            else:
                pair = self.socket.receive()
                logging.debug(f"Received garbled_value pair, garbled_value {b_input} selected")
                b_inputs_encr[w] = pair[b_input]

        result = GC.evaluate(circuit, g_tables,  a_inputs, b_inputs_encr)

        logging.debug("Sending circuit evaluation")
        self.socket.send(result)

    def ot_generator(self, msgs):
    ...
    def ot_hash(pub_key, msg_length):
        """Hash function for OT garbled_values."""
